...
function List({ board, data, postCard }) {
  ...
  const { move, setMove, fetchBoard } = useBoardContext();
  ...
  return (
    <div
      className={`board-list ${modalMode ? "up" : ""} ${ move.from && (move.from.id === data.id)? "moving" : ""}`}
      onClick={(e) => {
        if (move.bool) {
          // if state is 'moving'
          if (move.mode === "list") {
            ...
            apis.list
              ...
              .then((response) => fetchBoard({ key: board.key }))
